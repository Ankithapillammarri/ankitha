...
public class AdminDAOImplement implements AdminDAO{
	...
	/*
	 * @Override public boolean issueBook(int bId, int uId) { try { manager =
	 * factory.createEntityManager(); transaction = manager.getTransaction(); String
	 * jpql = "select b from BookBean b where b.bId=:bId"; TypedQuery<BookBean>
	 * query = manager.createQuery(jpql,BookBean.class); query.setParameter("bId",
	 * bId); BookBean rs = query.getSingleResult(); if(rs != null) { String jpql1 =
	 * "select r from RequestDetailsBean r where r.uId=:uId and r.bId=:bId";
	 * TypedQuery<RequestDetailsBean> query1 =
	 * manager.createQuery(jpql1,RequestDetailsBean.class);
	 * query1.setParameter("uId", uId); query1.setParameter("bId", bId);
	 * List<RequestDetailsBean> rs1 = query1.getResultList(); if(!rs1.isEmpty() &&
	 * rs1 != null) { transaction.begin(); SimpleDateFormat sdf = new
	 * SimpleDateFormat("yyyy-MM-dd"); Calendar cal = Calendar.getInstance(); String
	 * issueDate = sdf.format(cal.getTime()); cal.add(Calendar.DAY_OF_MONTH, 7);
	 * String returnDate = sdf.format(cal.getTime()); BookIssueBean issueBook = new
	 * BookIssueBean(); issueBook.setUId(uId); issueBook.setBId(bId);
	 * issueBook.setIssueDate(java.sql.Date.valueOf(issueDate));
	 * issueBook.setReturnDate(java.sql.Date.valueOf(returnDate));
	 * manager.persist(issueBook); transaction.commit(); if(!rs1.isEmpty() && rs1 !=
	 * null) { transaction.begin(); Query bookName =
	 * manager.createQuery("select b.bookName from BookBean b where b.bId=:bId");
	 * bookName.setParameter("bId", bId); List book = bookName.getResultList();
	 * BorrowedBooksBean borrowedBooks = new BorrowedBooksBean();
	 * borrowedBooks.setUId(uId); borrowedBooks.setBId(bId);
	 * borrowedBooks.setBookName(book.get(0).toString());
	 * manager.persist(borrowedBooks); transaction.commit(); return true; }else {
	 * throw new LMSException("Book Not issued"); } }else { throw new
	 * LMSException("The respective user have not placed any request"); } }else {
	 * throw new LMSException("There is no book exist with bookId"+bId); } }catch
